 ...
 *    - Array of domain can use the proxy
 *
 */

 EDGE_CACHE_TTL = EDGE_CACHE_TTL || 5
 BROWSER_CACHE_TTL = BROWSER_CACHE_TTL || 0
 PROVIDERS = JSON.parse(PROVIDERS)
 CORS_WHITELIST = JSON.parse(CORS_WHITELIST)
 ...
 function checkOrigin(origin) {
   ...
   let isAllow = false
   for (let index = 0; index < CORS_WHITELIST.length; index++) {
     const allowDomain = CORS_WHITELIST[index];
     if(origin.includes(allowDomain)){
       isAllow = true
       break;
     }
   }
 ...
 async function formatResponse (response, body) {
   ...
   formattedResponse.headers.set('Content-Type', 'application/json')
 ...
 async function handleOptions(event) {
   ...
   if (
     ...
     checkOrigin(headers.get("Origin")) &&
 ...
 async function handlePost(event) {
   ...
   const cacheKey = new Request(url.toString(), {
    ...
    console.log("request method -> ",method,hash,cacheable,cacheableBody)
 ...
 function HandleError(){
   let response = new Response("Request is invalid.",{"status":401})
   return response
 }
 ...
 addEventListener("fetch", event => {
   ...
   if (
     ...
     return event.respondWith(HandleError())
   }
 })